...
def get(request):
    ...
    try:
        ...
        return JsonResponse(
            fail_resp(code=DATABASE_ERROR, msg="Failed to get a entity type record.", data=get_exception(e)))
...
def exist_sentence(tag_id):
    try:
        ...
        return JsonResponse(
            fail_resp(code=DATABASE_ERROR, msg="Get number of entity tags failed !", data=get_exception(e)))
    # 该 tag_id 不存在
    if num == 0:
        return False
    ...
    # 只剩一条数据，此时更改标志位
        ...
        try:
            ...
            return JsonResponse(
                fail_resp(code=DATABASE_ERROR, msg="Change sentence entity tag flag faild!", data=get_exception(e)))
        return True
